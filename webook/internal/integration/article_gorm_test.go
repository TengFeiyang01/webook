	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				var publishedArt dao.PublishedArticleV1
				...
				publishedArt.Id = 0
				assert.Equal(t, dao.PublishedArticleV1{
					Article: dao.Article{
						Title:    "hello，你好",
						Content:  "随便试试",
						AuthorId: 123,
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				assert.NoError(t, err)
				part := dao.PublishedArticleV1{Article: art}
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				err := s.db.Create(&art).Error
				assert.NoError(t, err)
				part := dao.PublishedArticleV1{Article: dao.Article{
					Id:       4,
					Title:    "我的标题",
					Content:  "我的内容",
					Ctime:    456,
			...
			after: func(t *testing.T) {
				...
				var part dao.PublishedArticleV1
				// 数据没有变化
