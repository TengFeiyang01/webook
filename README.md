...
1. **负载均衡的定义**：通过某种手段将请求分发到最适合处理请求的节点，以达到快速响应的目的。
2. **负载均衡的目的**：挑选最适合处理请求的节点，而不是简单地根据负载最低来选择。
3. **负载均衡的简化模型**：通过量化节点来挑选负载最轻的节点。

### 负载均衡算法分类

#### 静态负载均衡算法

1. **轮询（Round Robin）**：简单且广泛应用的负载均衡算法，按顺序轮询节点分发请求。
   - **优点**：实现简单，适用于负载均衡需求不高的场景。
   - **缺点**：不考虑节点的实际负载情况，可能导致某些节点过载。

2. **随机（Random）**：随机选择节点分发请求，每次选择概率相同。
   - **优点**：实现简单，适用于节点性能相近的场景。
   - **缺点**：不考虑节点的实际负载情况，可能导致负载不均衡。

3. **哈希（Hash）**：根据业务特征计算哈希值，通过哈希值选择节点。
   - **优点**：可以根据特定的业务需求进行定制化选择。
   - **缺点**：需要设计合理的哈希函数，可能导致哈希碰撞。

4. **一致性哈希（Consistent Hashing）**：哈希算法的改进，能够更好地处理节点的动态增减。
   - **优点**：在节点增减时，能够最小化数据的重新分配。
   - **缺点**：实现复杂度较高。

#### 动态负载均衡算法

1. **最少连接数（Least Connections）**：选择连接数最少的节点，可能受连接多路复用影响。
   - **优点**：能够动态反映节点的负载情况。
   - **缺点**：在长连接场景下，可能不准确。

2. **最少活跃数（Least Active）**：选择活跃请求数最少的节点，可能受请求资源不同影响。
   - **优点**：能够更准确地反映节点的实时负载。
   - **缺点**：需要实时监控节点的活跃请求数，增加系统开销。

3. **最快响应时间（Fastest Response Time）**：选择响应时间最短的节点，可能受长尾效应影响。
   - **优点**：能够选择当前响应最快的节点，提高用户体验。
   - **缺点**：需要实时监控节点的响应时间，增加系统开销。

### 权重和动态调整

1. **权重（Weight）**：用于表达服务器的处理能力或重要性，权重高的节点可能连续被选中。
2. **平滑权重（Smoothing Weight）**：避免权重高的节点连续被选中，通过降低权重实现。
   - **详细介绍**：见上文。

3. **结合实际调用结果调整权重**：根据响应结果、错误率、响应时间调整权重。
   - **优点**：能够动态调整节点的权重，提高负载均衡的准确性。
   - **缺点**：需要实时监控和调整，增加系统复杂度。

4. **权重上限和下限**：避免权重变为零或负数。
   - **优点**：确保权重的合理性，避免极端情况。
   - **缺点**：需要合理设置上限和下限，避免影响负载均衡效果。

### 客户端和服务端的局限性

1. **客户端局限性**：只了解客户端和服务端的关系，不了解全局信息。
   - **影响**：可能导致负载均衡决策不准确。

2. **服务端局限性**：单个服务端实例不了解全局信息，多个实例之间不进行数据同步。
   - **影响**：可能导致负载不均衡。

3. **网关的优势**：单实例网关具备全局信息，可通过网关进行负载均衡。
   - **优点**：能够更好地进行全局负载均衡，提高系统性能。

### 自定义负载均衡算法

1. **根据业务特征选择指标**：错误率、CPU、IO、网络负载等。
   - **优点**：能够根据具体业务需求进行定制化负载均衡。

2. **通过注册中心、响应头、可观测性平台获取服务端数据**。
   - **优点**：能够获取实时的服务端状态，提高负载均衡的准确性。

3. **自定义负载均衡算法的设计**：结合业务特征和实际需求。
   - **优点**：能够根据具体业务场景进行优化，提高系统性能。
...
**熔断：在服务的依赖调用中，被调用方出现故障时，出于自我保护的目的，调用方会主动停止调用，并根据业务需要进行相应处理。调用方这种主动停止调用的行为我们称之为熔断。**

**限流：限流是针对服务请求数量的一种自我保护机制，当请求数量超出服务的处理能力时，会自动丢弃新来的请求。**

**降级：降级是通过开关配置将某些不重要的业务功能屏蔽掉，以提高服务处理能力。在大促场景中经常会对某些服务进行降级处理，大促结束之后再进行复原。**
...
- **读者**：发起支付。
- **webook**：作为枢纽，要记录打赏，要调用第三方接口发起支付。
- **第三方支付**：真正完成支付的地方。
- **创作者**：在最终时候能够收到打赏的金额

<img src="https://gcore.jsdelivr.net/gh/TengFeiyang01/picture@master/data/202504281102094.png" alt="image-20250428110240857" style="zoom: 33%;" />

**支付过程** 

- 读者点击打赏。
- webook 创建一个支付订单。
- webook 跳转到第三方支付（或者展示一个二维码）。
- 读者扫码支付。
- 第三方支付发送一个回调给 webook。
- webook 得知支付结果，记录结果并且更新系统状态
